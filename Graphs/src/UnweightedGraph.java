...
public class UnweightedGraph<V> implements IGraph<V> {
    ...
    private void dfs(int v, boolean[] visited){
        ...
        for (Integer u : getNeightbors(v)){ // v = 1 , u = 5,4,3,2
            // Er den bedsøgt? Hvis ikke, sæt den til at være det
            if(!visited[u]){
                dfs(u,visited);
                //dfs = deepth first search: går dybt ned i træet først
            }
        }
    }

    private List<Integer> getNeightbors(int u){
